...
class Field:
    ...
    def G(self, x, u):
        ...
        dxdx = np.array([1, 0, -trans * np.sin(theta)]).reshape((-1, 1))
        dydx = np.array([0, 1, trans * np.cos(theta)]).reshape((-1, 1))
        dzdx = np.array([0, 0, 1]).reshape((-1, 1))
        return np.concatenate((dxdx.T, dydx.T, dzdx.T), axis=0)
    ...
    def V(self, x, u):
        ...
        # YOUR IMPLEMENTATION HERE
        theta = prev_theta + rot1
        dxdu = np.array([-trans * np.sin(theta), np.cos(theta), 0]).reshape((-1, 1))
        dydu = np.array([trans * np.cos(theta), np.sin(theta), 0]).reshape((-1, 1))
        dzdu = np.array([1, 0, 1]).reshape((-1, 1))
        return np.concatenate((dxdu.T, dydu.T, dzdu.T), axis=0)

    def H(self, x, marker_id):
        """Compute the Jacobian of the observation with respect to the state."""
        prev_x, prev_y, prev_theta = x.ravel()
        # YOUR IMPLEMENTATION HERE
        dtdx = np.array([(self.MARKER_Y_POS[marker_id] -prev_y )/((self.MARKER_X_POS[marker_id] - prev_x)**2 + (self.MARKER_Y_POS[marker_id] - prev_y)**2), -(self.MARKER_X_POS[marker_id] - prev_x)/((self.MARKER_X_POS[marker_id] - prev_x)**2 + (self.MARKER_Y_POS[marker_id] - prev_y)**2), -1]).reshape((1, -1))
        return dtdx
